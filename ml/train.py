...
def load_and_prepare_data() -> tuple[pd.DataFrame, pd.Series]:
    ...
    merged_df = housing_df.merge(
        demographics_df[['zipcode', 'median_income', 'population_density', 'median_age']],
        on='zipcode',
        how='left'
    )
    ...
    feature_columns = [
        ...
        'floors', 'condition', 'grade', 'year_built',
        'median_income', 'population_density', 'median_age'
    ]
...
def train_model(X: pd.DataFrame, y: pd.Series) -> tuple[HousingPriceModel, dict]:
    ...
    logger.info(f"CV Mean R² Score: {cv_results['mean_cv_score']:.4f} (+/- {cv_results['std_cv_score']:.4f})")
    ...
    if train_r2 - test_r2 > 0.1:
        logger.warning("⚠️ Potential OVERFITTING detected! Train-Test R² gap: {:.4f}".format(train_r2 - test_r2))
        metrics['overfitting_warning'] = True
    elif test_r2 < 0.5:
        logger.warning("⚠️ Potential UNDERFITTING detected! Test R² is low: {:.4f}".format(test_r2))
