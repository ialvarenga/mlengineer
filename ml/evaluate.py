...
class ModelEvaluator:
    ...
    def print_report(self):
        ...
        print("-" * 50)
        print(
            f"{'R² Score':<20} {metrics['train']['r2_score']:.4f}         {metrics['test']['r2_score']:.4f}"
        )
        print(
            f"{'RMSE':<20} ${metrics['train']['rmse']:,.0f}       ${metrics['test']['rmse']:,.0f}"
        )
        print(
            f"{'MAE':<20} ${metrics['train']['mae']:,.0f}       ${metrics['test']['mae']:,.0f}"
        )
        print(
            f"{'MAPE':<20} {metrics['train']['mape']:.2f}%         {metrics['test']['mape']:.2f}%"
        )
        ...
        print(
            f"Status: {status_emoji.get(analysis['status'], '❓')} {analysis['status'].upper()}"
        )
        print(f"\n{analysis['description']}")
        ...
        for i, suggestion in enumerate(results["improvement_suggestions"][:5], 1):
            ...
            print(
                f"{i}. [{priority_emoji[suggestion['priority']]}] {suggestion['category']}"
            )
