...
def load_secrets_from_aws(secret_name: Optional[str] = None) -> bool:
    ...
    if not use_aws_secrets:
        logger.info("AWS Secrets Manager disabled (USE_AWS_SECRETS != 'true'). Using environment variables.")
        return False
    
    client = get_secrets_manager_client()
    if not client:
        logger.warning("Could not create Secrets Manager client. Using environment variables.")
        return False
    ...
    try:
        ...
        if "AccessDenied" in error_msg:
            ...
            logger.error("AWS credentials not found. Configure credentials via environment variables or AWS CLI.")
        else:
