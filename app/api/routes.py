...
async def predict_price_minimal(
    house: MinimalHouseFeatures, user: User = Depends(require_auth)
):
    """
    Predict the price of a house using only the required features.

    This endpoint requires only the features that the ML model actually uses,
    making it simpler to call when you don't have all the house details.
    ...
    **Required Features:**
    ...
    - floors: Number of floors
    - sqft_above: Square feet above ground
    - sqft_basement: Square feet of basement (default: 0)
    - zipcode: Zip code (for demographic data lookup)
...
async def predict_batch(
    request: BatchPredictionRequest, user: User = Depends(require_auth)
):
    """
