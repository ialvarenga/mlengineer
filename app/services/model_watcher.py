...
class ModelWatcher:
    ...
    async def check_and_reload(self) -> dict:
        ...
        if current_version == latest_version:
            ...
            result["reason"] = (
                f"Current version {current_version} is already the latest"
            )
            logger.debug(f"Model check: version {current_version} is current")
            return result

        logger.info(
            f"New model version detected: {latest_version} (current: {current_version})"
        )
        ...
        try:
            ...
            result["reason"] = (
                f"Successfully reloaded from {current_version} to {prediction_service._loaded_version}"
            )

            logger.info(
                f"Model hot-reloaded to version: {prediction_service._loaded_version}"
            )
    ...
    async def _watch_loop(self):
        ...
        logger.info(
            f"Model watcher started (interval: {MODEL_REFRESH_INTERVAL_SECONDS}s)"
        )
    ...
    async def start(self):
        ...
        if not ENABLE_MODEL_AUTO_REFRESH:
            logger.info(
                "Model auto-refresh is disabled (ENABLE_MODEL_AUTO_REFRESH=false)"
            )
            return
